...
the free Anaconda Python distribution from Continuum Analytics
(continuum.io), as the most stable and trouble free distribution on
all platforms, especially, Mac.

NOTE: These programs may run under Python 3 at the present time, but
...
There is no plan yet for a formal Python 3 version, but conversion
should be straightforward with the latest version as we've been
careful about using print, except, etc. so that we're already close.
